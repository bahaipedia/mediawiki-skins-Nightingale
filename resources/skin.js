/* JavaScript for the Ridvan skin */
$( function () {
    // 1. Handle "Click Outside" to close menus
    $( document ).on( 'click', function ( e ) {
        // Check if the click happened INSIDE a portlet (menu container)
        if ( !$( e.target ).closest( '.mw-portlet' ).length ) {
            // If not, find ALL checkboxes inside portlets and uncheck them
            $( '.mw-portlet input[type="checkbox"]' ).prop( 'checked', false );
        }
    } );
    // 2. Handle "Exclusive" behavior (Close others when one opens)
    $( '.mw-portlet input[type="checkbox"]' ).on( 'change', function () {
        // If this specific menu was just opened...
        if ( $( this ).is( ':checked' ) ) {
            // ...find all OTHER checkboxes and close them.
            $( '.mw-portlet input[type="checkbox"]' ).not( this ).prop( 'checked', false );
        }
    } );
} );
